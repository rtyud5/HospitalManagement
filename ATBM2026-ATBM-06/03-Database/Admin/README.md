# Admin/ - Phân hệ 1: Quản trị CSDL Oracle
...
toàn dự án dùng chung một folder script (trong nộp bài là `03-Database/`), cùng cấp với
`RBAC/`, `VPD/`, `Audit/`, `OLS/` phục vụ Phân hệ 2.
...
- `RBAC/` - roles `RL_DIEUPHOIVIEN`, `RL_BACSI`, `RL_KYTHUATVIEN`, `RL_BENHNHAN`
- `VPD/` - chính sách VPD theo vai trò
- `Audit/` - audit trigger cho kết quả xét nghiệm
- `OLS/` - OLS cho bảng THONG_BAO

Xem `README.md` trong cùng thư mục `03-Database` để biết thứ tự chạy tổng thể cho Phân hệ 2
